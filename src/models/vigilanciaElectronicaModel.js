  ...
  create: async function(vigilanciaElectronicaData) {
    ...
    try {
      const [result] = await pool.execute(sql, [
        ...
        vigilanciaElectronicaData.totalE
      ]);
      if (result.affectedRows === 1) {
        const nuevaVigilanciaElectronica = {
          ...
          totalE: vigilanciaElectronicaData.totalE
        };
...
async function deleteByIdVigilanciaElectronica(idvigilancia_electronica) {
  try {
    ...
    if (result.affectedRows === 0) {
      throw new Error('La vigilancia electrónica seleccionada no existe');
    }
    return { message: 'Vigilancia electrónica eliminada exitosamente' };
  } catch (error) {
    throw error;
  }
}
